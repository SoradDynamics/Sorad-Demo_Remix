...
import { getSchoolDomain } from './schoolStore'; // Adjust path if necessary
...
const APPWRITE_DATABASE_ID = DB_Id;
...
export const useTransportStore = create<TransportState>((set, get) => ({
    ...
    addDriverData: async (driverFormData) => {
        if (!API_BASE_URL || !APPWRITE_DATABASE_ID || !DRIVER_COLLECTION_ID || !AUTH_API_URL) {
            const errorMsg = "URL/Database/Collection ID or Auth API URL missing in config for adding driver.";
            set({ isLoading: false, error: errorMsg });
            // throw new Error(errorMsg); // Consider returning null for consistency
            return null;
        }
        set({ isLoading: true, error: null });
        let createdAuthUserId: string | null = null;

        // Get school domain from schoolStore
        ...
        try {
            ...
            const signupPayload = {
                ...
                labels: ['driver'],
                domain: schoolDomain, // <-- ADDED: Send domain to backend
            };

            // console.log(`Attempting signup via backend for: ${driverFormData.email} with domain: ${schoolDomain}`);
            const signupResponse = await fetch(`${AUTH_API_URL}/signup`, { // Ensure AUTH_API_URL is correctly defined
                method: 'POST',
            ...
            const addDocumentPayload = {
                ...
                // schoolDomain: schoolDomain, // You could also store it here if needed for direct DB queries
            };
            ...
            if (createdAuthUserId && error.message && !error.message.includes('Auth user creation failed')) {
                ...
                try {
                    // Ensure your backend delete endpoint is correctly configured
                    const rollbackResponse = await fetch(`${AUTH_API_URL}/user/${createdAuthUserId}`, { method: 'DELETE' }); // Assuming endpoint is /user/:id
