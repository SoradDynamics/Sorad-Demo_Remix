...
const ExamCard: React.FC<ExamCardProps> = ({
  ...
  const currentPercentage: number | null =
    entryPercentage === undefined ? null : entryPercentage;
  ...
  return (
    ...
    >
      <CardHeader className=" px-5 border-b flex flex-col border-gray-100 bg-gray-50">
        <div className="flex justify-between items-start w-full">
          <h4
            className="font-bold text-lg text-gray-900 truncate mr-2"
            title={exam.title}
          >
            {exam.title}
          </h4>
          <div className="flex flex-col sm:flex-row items-end sm:items-center gap-2 flex-shrink-0">
            {exam.isPublished && (
              <Chip
                size="sm"
                color={exam.isGpa ? "success" : "primary"}
                variant="flat"
              >
        ...
        <div className="w-full text-left">
          {exam.desc && <p className="text-xs text-gray-500">{exam.desc}</p>}
        </div>
      </CardHeader>
      <CardBody className="p-5 space-y-3">
        ...
        <div className="flex items-center text-sm text-gray-600">
          ...
          <span>
            Faculties:{" "}
            <span className="font-medium text-gray-800">
              {facultyNames.length > 0 ? facultyNames.join(", ") : "All"}
            </span>
          </span>
        </div>
        <div className="flex items-center text-sm text-gray-600">
