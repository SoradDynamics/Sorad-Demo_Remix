...
import {
  ...
  Button,
  useDisclosure,
} from "@heroui/react"; // Import necessary components from Hero UI
import { usePopover } from "~/hooks/usePopover"; // Assuming you have a usePopover hook
...
const DriverIconList: React.FC<DriverIconProps> = ({
...
const DriverComponent: React.FC = () => {
  ...
  const [currentUserAppwriteId, setCurrentUserAppwriteId] = useState<
    string | null
  >(null);
  const [driverDocId, setDriverDocId] = useState<string | null>(null); // $id of the document in 'coll-drivers'

  const { isOpen, targetRef, openPopover, closePopover } = usePopover(); // Use the hook

  const [isPopoverOpen, setIsPopoverOpen] = useState(false);
  const [isConfirmLoading, setIsConfirmLoading] = useState(false);

  // Toggle the popover open/close
    ...
    setDriverColors((prevColors) => {
      ...
      locations.forEach((loc) => {
        if (loc.id && !newColors[loc.id]) {
          // Check if ID exists and color not already generated
  ...
  useEffect(() => {
    ...
    return () => {
      ...
      if (watchIdRef.current !== null) {
        ...
        // // console.log(
        //   "Cleared geolocation watch on unmount:",
        //   watchIdRef.current
        // );
  ...
  useEffect(() => {
    const fetchUser = async () => {
      ...
      try {
        ...
        if (isMountedRef.current) {
          setError(
            `Could not get driver identity: ${
              err.message || "Please ensure you are logged in."
            }`
          );
          setStatus("Error");
        }
  ...
  const findDriverDocumentByEmail = useCallback(
    async (email: string, expectedName: string) => {
      ...
      try {
        ...
        if (response.documents.length > 0) {
          ...
          console.error(
            `No document found in collection '${COLLECTION_ID}' for email: ${email}`
          );
          if (isMountedRef.current) {
            setError(
              `Configuration error: No driver document found for your email (${email}). Please create one or contact support.`
            );
  ...
  useEffect(() => {
    ...
    currentUserName,
    driverDocId,
    findDriverDocumentByEmail,
  ]);

  // --- 3. Update Location in the Found Appwrite Document ---
  const updateLocationInAppwrite = useCallback(
    async (latitude: number, longitude: number) => {
      ...
      try {
        ...
        if (isMountedRef.current) {
          ...
          setStatus((prev) =>
            prev.startsWith("Geolocation Error")
              ? prev
        ...
        console.error(
          `Failed to update location in document ${driverDocId}:`,
          err
        );
        if (isMountedRef.current) {
          setError(
            `Sync Error: ${
              err.message || "Could not update location in database"
            }`
          );
  ...
  useEffect(() => {
    ...
    if (!driverDocId || !navigator.geolocation) {
      if (!navigator.geolocation && driverDocId && isMountedRef.current) {
        // Only show error if doc ID was found but geo is missing
    ...
    const successCallback: PositionCallback = (position) => {
      ...
      setAllLocations((prev) => {
        ...
        if (idx > -1) {
          // Update existing entry for self
          const updated = [...prev];
          updated[idx] = selfLocationData;
          return updated;
        } else {
          // Add self if not present (should be rare if fetch worked)
  ...
  useEffect(() => {
    ...
    const parseAndValidateLocation = (doc: DriverDoc): LocationData | null => {
      ...
      if (isNaN(lat) || isNaN(lon)) {
        console.warn(
          `Invalid coordinates in DB doc ${doc.$id}: lat='${doc.latitude}', lon='${doc.longitude}'. Skipping.`
        );
        return null;
      }
      return {
    ...
    const fetchAndSubscribe = async () => {
      ...
      setStatus((prev) =>
        prev.includes("active") || prev.includes("found")
          ? `${prev} Fetching locations...`
          : "Fetching locations..."
      );
      setError(null);
      try {
        ...
        setStatus((prev) =>
          prev.includes("active") ? prev : "Subscribing to updates..."
        ); // Adjust status
          ...
          (response: any) => {
            ...
            setAllLocations((prev) => {
              ...
              if (event.includes(".delete")) {
                ...
                // Only update/add if parsing was successful
                const idx = prev.findIndex((loc) => loc.id === locationId);
                if (idx > -1) {
                  // Update existing
                  newState = [...prev];
                  newState[idx] = processedLocation;
                } else {
                  // Add new
        ...
        setStatus((prev) =>
          prev.includes("active") || prev.includes("Error")
            ? prev
  ...
  const [isDriverListExpanded, setIsDriverListExpanded] = useState(true);

  const panelHeight = isDriverListExpanded ? "h-84" : "h-[7.3rem]";
  ...
  return (
    <div className="flex flex-col md:flex-row h-screen bg-gray-100">
      ...
      >
        {/* Header section */}
        <div className="p-3 bg-blue-100 border border-blue-200 shadow-sm mb-4 rounded-md flex items-center justify-between">
          {" "}
          {/* Flex container */}
          <div>
            {/* Toggle Button */}
            <button
              className="p-2 absolute sm:hidden right-5 rounded-full   hover:bg-gray-300 focus:outline-none" /* Circular button */
              onClick={() => setIsDriverListExpanded(!isDriverListExpanded)}
              aria-label={
                isDriverListExpanded
                  ? "Collapse Driver List"
                  : "Expand Driver List"
              } //Accessibility
            >
              {isDriverListExpanded ? (
            ...
            {currentUserName && (
              <p className="text-sm text-gray-700 break-words">
                Driver: <span className="font-medium">{currentUserName}</span>
              </p>
            )}
            {currentUserEmail && (
            ...
            {error && (
              <p className="text-red-700 bg-red-100 p-2 rounded text-xs mt-2 border border-red-200">
                Error: {error}
              </p>
            )}
          </div>
          <Button
            ...
            color="danger"
            mb-5
            onClick={() => { handleOpenPopover(); setIsDriverListExpanded(true); }}
            //   ref={targetRef}
          >
            {" "}
            Logout{" "}
        ...
        <div className="bg-white shadow rounded-lg overflow-hidden">
          <h3 className="text-md font-semibold p-3 bg-gray-100 border-b border-gray-200">
            Active Drivers
          </h3>
          {sortedDrivers.length === 0 ? (
            <div className="p-4 text-center text-sm text-gray-500">
              No other drivers currently active or available.
            </div>
          ) : (
            <ul className="divide-y divide-gray-200 max-h-[calc(100vh-280px)] overflow-y-auto">
              {" "}
              {/* Adjust max-h dynamically or fixed */}
              {sortedDrivers.map((driver) => (
                ...
                >
                  ...
                  <div className="flex-shrink-0">
                    <DriverIconList
                      color={driverColors[driver.id] || "gray"}
                      size={30}
                    />
                  </div>
                  {/* Driver details */}
