...
// Main assignment store
import { useAssignmentStore, Assignment } from '~/store/assignmentStore'; // Adjusted path
// New store for student submissions
import { useStudentSubmissionStore } from '~/store/studentSubmissionStore'; // Import the new store

import {
    FiPlus, FiCalendar, FiBookOpen, FiDownload, FiEdit, FiTrash2,
    FiPaperclip, FiFilter, FiLoader, FiAlertTriangle, FiUsers
} from 'react-icons/fi';
...
import AssignmentSubmissionsManager from './AssignmentSubmissionsManager';
import { FcAbout } from 'react-icons/fc';
import { getCurrentUser } from './utils/appwriteAuth';
import { Models } from 'appwrite';

// Helper to group assignments by date
const groupAssignmentsByDate = (assignments: Assignment[]): Record<string, Assignment[]> => {
            ...
            <div className="bg-white p-6 rounded-lg shadow-xl max-w-sm w-full">
                ...
                <div className="flex justify-end space-x-3">
                    ...
                    <button
                        onClick={onConfirm}
                        disabled={isConfirmLoading}
                        className="px-4 py-2 text-sm font-medium text-white bg-red-600 hover:bg-red-700 rounded-md shadow-sm disabled:opacity-50 flex items-center justify-center min-w-[100px]"
                    >
...
const TeacherAssignmentsPage: React.FC = () => {
    // From main assignment store for assignment list, CRUD, filters
    const {
    ...
    // Get state and actions for the submissions manager from its own store
    const isSubmissionsManagerDrawerOpen = useStudentSubmissionStore(state => state.isSubmissionsManagerOpen);
    const openSubmissionsManagerAction = useStudentSubmissionStore(state => state.openSubmissionsManager); // <-- CORRECTED: Get action directly
    ...
    // This function now correctly calls the action from useStudentSubmissionStore
    const handleOpenSubmissionsManager = (assignment: Assignment) => {
        openSubmissionsManagerAction(assignment);
    };
            ...
            <div className="container mx-auto max-w-7xl">
                <header className="mb-6 md:mb-8 flex flex-col sm:flex-row justify-between items-center gap-4">
                    ...
                    {!loggedInUser && !authLoading && (
                         <p className="text-sm text-orange-600 bg-orange-100 p-2 rounded-md">Please log in to manage assignments.</p>
                    )}
                </header>
                ...
                {!isLoading && !pageError && assignments.length === 0 && (
                    <div className="text-center py-16 text-slate-500 bg-white rounded-xl shadow-lg p-8">
                        ...
                        <p className="text-slate-600 mt-2">
                            There are no assignments matching your current filters. <br/>
                            {loggedInUser ? "You can add a new assignment using the button above." : "Please log in to add assignments."}
                        </p>
                    </div>
                )}
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
                            {assignmentsOnDate.map(assignment => (
                                <div key={assignment.$id} className="bg-white rounded-xl shadow-lg hover:shadow-2xl transition-shadow duration-300 flex flex-col overflow-hidden group">
                                                ...
                                                <div className="space-y-2">
                                                    {assignment.fileIds.map((fileId, index) => (
                                                        <button
                                                            ...
                                                            className="w-full flex items-center text-sm text-sky-700 hover:text-sky-900 bg-sky-50 hover:bg-sky-100 p-2.5 rounded-md transition-colors border border-sky-200 hover:border-sky-300 focus:outline-none focus:ring-2 focus:ring-sky-500 focus:ring-offset-1"
                                                        >
                                    ...
                                    {loggedInUser && loggedInUser.$id === assignment.assignedById && (
                                        <div className="bg-slate-50 p-3 border-t border-slate-200 flex flex-wrap justify-end gap-2">
                                             <button
                                                onClick={() => handleOpenSubmissionsManager(assignment)}
                                                className="text-xs font-medium text-purple-600 hover:text-purple-800 py-1.5 px-3 rounded-md hover:bg-purple-100 transition-colors flex items-center focus:outline-none focus:ring-1 focus:ring-purple-500"
                                                aria-label={`Manage submissions for: ${assignment.title}`}
                                            >
                                                <FiUsers className="mr-1.5 h-3.5 w-3.5" /> Manage Submissions
                                            </button>
                                            <button
                                            ...
                                            >
                                                <FiEdit className="mr-1.5 h-3.5 w-3.5" /> Edit Details
                                            </button>
                                            <button
                ...
                {isSubmissionsManagerDrawerOpen && <AssignmentSubmissionsManager />}
