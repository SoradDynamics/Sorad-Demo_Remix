...
import { useSchoolStore } from "~/store/schoolStore"; // <-- ADD THIS IMPORT
...
const List: React.FC<ListProps> = ({
  ...
  const { domain: schoolDomain } = useSchoolStore(); // <-- ADD THIS LINE to get domain
  ...
  >([]);
  const [filteredAddModalSectionItems, setFilteredAddModalSectionItems] =
    useState<{ key: string; label: string }[]>([]);
  ...
  const handleAddSaveNewStudent = async () => {
    ...
    if (!schoolDomain) {
      setAddError("School domain is not configured. Cannot create student. Please re-login or contact support.");
      setIsSaving(false);
      toast.error("School domain missing. Action aborted.");
      return;
    }
    ...
    try {
      ...
      if (!isExistingParentCheckboxChecked) {
        ...
        const createdParent = await addParentData(
          newParentDataForDb,
          parentUserId
        );
        if (!createdParent) {
      ...
      if (newStudentSection) {
        ...
        if (selectedSectionObject) {
          ...
          console.warn(
            `Could not find section name for ID: ${newStudentSection}. Saving empty section name.`
          );
        }
      }
    ...
    if (searchText) {
      ...
      const sortedData = [...fullStudentData].sort((a, b) => {
        try {
          ...
          console.error(
            "Error parsing date for sorting student data:",
            e,
            a,
            b
          );
  ...
  return (
    <div className="md:w-full w-full md:p-2">
      ...
      >
        <Drawer.Header showCloseButton={!isSaving}>
          Add New Student
        </Drawer.Header>
        <Drawer.Body>
          {isSaving && (
          ...
          >
                      ...
                      filteredParentItemsForDropdown.length > 0 ? (
                        filteredParentItemsForDropdown.map((item) => (
                          <li
                            ...
                            role="option"
                            aria-selected={
                              selectedExistingParentId === item.key
                            }
            ...
            <Autocomplete
              ...
              onInputChange={(value) => {
                ...
                setFilteredAddModalFacultyItems(
                  filtered.length > 0
                    ? filtered
                    : facultyData.map((f) => ({ key: f.$id, label: f.name }))
                );
              }}
              ...
              onInputChange={(value) => {
                ...
                setFilteredAddModalClassItems(
                  filteredClasses.length > 0
                    ? filteredClasses
                    : allClassesForSelectedFaculty.map((cls) => ({
                        key: cls,
                        label: cls,
                      }))
                );
            ...
            <Autocomplete
              ...
              onInputChange={(value) => {
                const sectionsForClassAndFaculty = sectionData.filter(
                  (section) =>
                    section.facultyId === selectedFacultyId &&
                    section.class === newStudentClass
                );
                ...
                setFilteredAddModalSectionItems(
                  filteredSections.length > 0
                    ? filteredSections
                    : sectionsForClassAndFaculty.map((s) => ({
                        key: s.$id,
                        label: s.name,
                      }))
                );
