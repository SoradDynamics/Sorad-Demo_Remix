...
// Move interface to Details component file and use FC type
// type Props = { // This was commented out in your original; if needed, it belongs in Details.tsx
//   studentIds: string[];
//   onBack?: () => void;
// }
...
const Attendance = () => {
  ...
  const handleStudentsSelect = useCallback((studentIds: string[]) => {
    setSelectedStudents(studentIds); // Update the selected students state (This is line 36)
    ...
    // else if (isMobile && studentIds.length === 0 && showDetailsMobile) { // Ensure not to toggle if already on list
